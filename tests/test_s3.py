...
class TestS3(unittest.TestCase):
    ...
    def test_s3_del_by_all(self):
        with patch("boto3.client") as cl:
            ...
            now = datetime.now()
            c = [{"LastModified": now-timedelta(x), "Size": x*1000, "Key": f"obj{x}"}
                 for x in range(10)]
            cl.return_value.list_objects.return_value = {
                "IsTruncated": False,
                "Contents": c,
    ...
    def test_s3_del_by_older(self):
        with patch("boto3.client") as cl:
            ...
            res = CliRunner().invoke(cli, ["s3-delete-by", "--s3-bucket", "bucket123", "--older", "3d"], env=self.envs)
            if res.exception:
                raise res.exception
            self.assertEqual(0, res.exit_code)
            cl.return_value.delete_objects.assert_called_once_with(
    ...
    def test_s3_del_by_older_dry(self):
        with patch("boto3.client") as cl:
            ...
            res = CliRunner().invoke(cli, ["s3-delete-by", "--s3-bucket",
                                           "bucket123", "--older", "3d", "--dry"], env=self.envs)
            if res.exception:
                raise res.exception
            self.assertEqual(0, res.exit_code)
    ...
    def test_s3_del_by_newer(self):
        with patch("boto3.client") as cl:
            ...
            res = CliRunner().invoke(cli, ["s3-delete-by", "--s3-bucket", "bucket123", "--newer", "3d"], env=self.envs)
            if res.exception:
                raise res.exception
            self.assertEqual(0, res.exit_code)
            cl.return_value.delete_objects.assert_called_once_with(
    ...
    def test_s3_del_by_older_newer(self):
        with patch("boto3.client") as cl:
            ...
            res = CliRunner().invoke(cli, ["s3-delete-by", "--s3-bucket", "bucket123",
                                           "--newer", "7d", "--older", "2d"], env=self.envs)
            if res.exception:
                raise res.exception
            self.assertEqual(0, res.exit_code)
            cl.return_value.delete_objects.assert_called_once_with(
    ...
    def test_s3_cat_gz(self):
        ...
        with patch("boto3.client") as cl:
            ...
            res = CliRunner().invoke(cli, ["s3-cat", "--s3-bucket", "bucket123",
                                           "path/to/hello.gz"], env=self.envs)
            if res.exception:
                raise res.exception
            self.assertEqual(0, res.exit_code)
    ...
    def test_s3_less_bz2(self):
        ...
        with patch("boto3.client") as cl, \
            ...
            res = CliRunner().invoke(cli, ["s3-less", "--s3-bucket", "bucket123",
    ...
    def test_s3_vi_bz2(self):
        ...
        with patch("boto3.client") as cl, \
            ...
            res = CliRunner().invoke(cli, ["s3-vi", "--s3-bucket", "bucket123",
                                           "path/to/hello.bz2"], env=self.envs)
            if res.exception:
                raise res.exception
            self.assertEqual(0, res.exit_code)
    ...
    def test_s3_vi_xz_unchanged(self):
        ...
        with patch("boto3.client") as cl, \
            ...
            ed.return_value = moddata
            res = CliRunner().invoke(cli, ["s3-vi", "--s3-bucket", "bucket123",
                                           "path/to/hello.xz"], env=self.envs)
            if res.exception:
                raise res.exception
            self.assertEqual(0, res.exit_code)
    ...
    def test_s3_vi_changed_dry(self):
        ...
        with patch("boto3.client") as cl, \
            ...
            ed.return_value = moddata
            res = CliRunner().invoke(cli, ["s3-vi", "--s3-bucket", "bucket123",
                                           "path/to/hello.log", "--dry"], env=self.envs)
            if res.exception:
                raise res.exception
            self.assertEqual(0, res.exit_code)
            cl.return_value.put_object.assert_not_called()
